...
500 BANK0:ID=DB+OS:VP=AP:VR=0
510 S=TI:FOR Y=0 TO (BH/2)-1
520  FOR X=0 TO (BW/4)-1
...
525   I=PEEK(ID):I1=PEEK(ID+1)
530   P1$=MID$(HEX$(I),3,1)   :P2$=RIGHT$(HEX$(I),1):P3$=MID$(HEX$(I1),3,1):P4$=RIGHT$(HEX$(I1),1)
540   C1$=P1$:C2$=RIGHT$(C1$,1)
...
580   B=0
590   IF P2 THEN B=48:  C2$=P2$
600   IF P3 THEN B=B+12:C2$=P3$
610   IF P4 THEN B=B+3: C2$=P4$

620   VMW VR,B

625   I=PEEK(ID+BW/2):I1=PEEK(ID+BW/2+1)
630   P5$=MID$(HEX$(I),3,1):P6$=RIGHT$(HEX$(I),1):P7$=MID$(HEX$(I1),3,1):P8$=RIGHT$(HEX$(I1),1)
...
680   B=0
690   IF P5 THEN B=192 :C2$=P5$
700   IF P6 THEN B=B+48:C2$=P6$
710   IF P7 THEN B=B+12:C2$=P7$
720   IF P8 THEN B=B+3: C2$=P8$

730   VMW VR+80,B

740   VMW VP,DEC(C1$+C2$)
...
790 NEXT:S=TI-S:PRINT S
800 VTR 0,DB+IS,24000:S=TI-S:PRINT S
...
940 DEF FN W(ZZ)=PEEK(ZZ)+PEEK(ZZ+1)*256
950 DEF FN W4(ZZ)=PEEK(ZZ)+PEEK(ZZ+1)*256+PEEK(ZZ+2)*65536+PEEK(ZZ+3)*16777216
960 DEF FN V(ZZ)=DEC(RIGHT$(HEX$(ZZ),1))
970 DEF FN HN(ZZ)=DEC(MID$(HEX$(ZZ),3,1))
980 DEF FN LN(ZZ)=DEC(RIGHT$(HEX$(ZZ),1))
